...
async function listService () {
  ...
  if (!rates && !time) {
    const data = JSON.parse(await getRequest('https://api.exchangeratesapi.io/latest?base=USD')).rates; 
  ...
  if (timeToUpdate) {
    const newData = JSON.parse(await getRequest('https://api.exchangeratesapi.io/latest?base=USD')).rates;
